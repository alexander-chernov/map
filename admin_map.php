            ...
            $placeMarkAddress[$index] = "
                addressItem[".$index."] = new ymaps.Placemark(".$p2.",{
                    ...
                    balloonContentBody: '<p class=result>'+
                                        ...
                                        'Адресов:<a href=\'#\' onclick=\"showRightAddressByLink(".$link.")\">".$row['addr_count']."</a><br>'+
                                        'Предложений:<a href=\'#\' onclick=\"showRightRealtyByLink(".$link.")\">".$row['realty_sell']."</a><br>'+
                                        'Организаций:<a href=\'#\' onclick=\"showRightOrgsByLink(".$link.")\">".$row['org_count']."</a>'+
                                        '</p>'
                },{
            ...
            if (count($orgs)>0 && is_array($orgs)){
                ...
                while ($row = $res->fetch_assoc()) {
                    ...
                    var textBody = '<p class=address>'+
                                ...
                                '<p class=result>'+
                                'Адресов:<a href=\'#\' onclick=\"showRightAddressByLink(".$link.")\">".$addrCount."</a><br>'+
                                'Предложений:<a href=\'#\' onclick=\"showRightRealtyByLink(".$link.")\">".$realty_sell."</a><br>'+
                                'Организаций:<a href=\'#\' onclick=\"showRightOrgsByLink(".$link.")\">".$row['org_count']."</a>'+
...
while ($rowB = $resB->fetch_assoc()) {
    ...
    balloon".$i.".setData({content: '<a target=_blank href=\'".$rowB['url']."\'><img src=\'/banner/".$rowB['img']."\'></a>'});
    ";
    $i++;
}
        ...
        ymaps.geocode(coords).then(function (res) {
            ...
            $.ajax({
                ...
                success: function (data) { // вешаем свой обработчик на функцию success
                    ...
                    if (!data.house) {
                        textBody += '<br><b>' + names[0] + '</b> <a onclick="showStreetByLink(\''+encodeURIComponent(JSON.stringify(res_arr))+'\')" href="#">Посмотреть улицу на карте</a>';
                    } else {
                        textBody += '<br><b>' + names[0] + ' </b>';
                    }

                    if (data.district) {
                        textBody += '<br>Район: <b>' + data.district + ' <a href="#" onclick=\"showRightAddressByLink(\''+data.link+'\')\">' + data.district_count + '</a></b>';
                    }
                    if (data.massive) {
                        textBody += '<br>Микрорайон: <b>' + data.massive + ' <a href="#" onclick=\"showRightAddressByLink(\''+data.link+'\')\">' + data.massive_count + '</a></b>';
                    }
                    textBody += '</p>';
		            var link = 'a=' + coords[0].toPrecision(6) + ',' + coords[1].toPrecision(6) + data.link;
                    textBody += '<p class=result>';
                    if (!data.house) {
                        textBody += 'Адресов: <b><a href="#" onclick=\"showRightAddressByLink(\''+link+'\')\">' + data.street_count + '</a></b><br>';
                    }
                    textBody += 'Предложений: <b><a href="#" onclick=\"showRightRealtyByLink(\''+link+'\')\">' + data.count + '</a></b><br>'+
                                'Организаций: <b><a href="#" onclick=\"showRightOrgsByLink(\''+link+'\')\">' + data.count_f + '</b></b>';
                    ...
                    $("#closeBalloon").click(function(){
                        showRightMapByLink("" + data.link + '&a=' + coords[0].toPrecision(6) + ',' + coords[1].toPrecision(6) + "");
                        myMap.balloon.close();
                     });
                },
                error: function (data) {
            ...
            res.geoObjects.each(function (obj) {
                ...
                $.ajax({
                    ...
                    success: function (data) { // вешаем свой обработчик на функцию success
                        ...
                        textBody += '<p class=result>'+
                                    'Предложений: <b><a href="#" onclick=\"showRightRealtyByLink(\''+link+'\')\">' + data.count + '</a></b><br>'+
                                    'Организаций: <b><a href="#" onclick=\"showRightOrgsByLink(\''+link+'\')\">' + data.count_f + '</b></b>';
                        textBody = textBody + '</p>';
...
<body style="padding: 0px; margin: 0px;" id="body">
    <div id="map" style="clear:left; width:800px; height:475px;float:left;"></div>
</body>
</html>
